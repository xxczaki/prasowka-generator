...
import blacklistedWords from './utils/blacklist';
import {getArticles} from './utils/scraper';
...
const Utility = () => {
	...
	const handleSubmit = async e => {
		...
		if (values.region === 'poland') {
			newsapi.v2.topHeadlines({
				...
				const articles = response.articles.slice(0, values.category === 'politics' ? 40 : 15);

				if (values.category === 'business') {
					const formatted = articles.map(e => {
						const source = e.title.split('-').pop();

						return (
							<div key={e.title} style={{width: '80%'}}>
								<h3>{e.title.replace(source, '')} <u>{source}</u></h3>
								<br/>
					...
					const sortedArticles = articles.filter(e => {
						return categorizeNews(e.title) === values.category && !blacklistedWords.some(el => e.title.toLowerCase().includes(el));
					});
	...
	return (
        ...
        Region
				...
				<br/>
				{formState.values.region === 'poland' ?
					<>
						<label>
        Kategoria
							<br/>
							<Select required={formState.values.region === 'poland'} {...select('category')}>
								<option value="">Wybierz</option>
